...
export interface BaseTransaction {
    id?: number; // Optional: used for mock data/frontend IDs. MongoDB will use _id
    amount: number;
    currency: Currency;
    date: string; // ISO 8601 string (YYYY-MM-DD)
    description: string;
    origin?: string; // This might map to ITransaction.source or be a separate detail
    linkedTransactionId?: number; // Frontend ID for linking
}
...
export interface Expense extends BaseTransaction {
    ...
    linkedIncomeId?: number; // Frontend ID specific for an expense that has a refund
}
...
export interface Income extends BaseTransaction {
    ...
    linkedExpenseId?: number; // Frontend ID specific for an income that is a refund for an expense
...
export type ExpenseCategory = 'Survival' | 'Growth' | 'Fun' | 'Restaurants' | 'Mobility' | 'Groceries' | 'Other';
export type IncomeCategory = 'Salary' | 'Investment' | 'Gift' | 'Refund' | 'Other'; // These are more for the 'category' field on income